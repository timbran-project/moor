...
**Authentication** — Authenticated endpoints require an `X-Moor-Auth-Token` header containing a
PASETO v4.public token obtained from `/auth/connect` or `/auth/create`. Some endpoints also require
`X-Moor-Client-Token` and `X-Moor-Client-Id` headers for session continuity.

**FlatBuffers note** — FlatBuffer request/response bodies are opaque binary blobs whose schemas are
defined in the `moor-schema` crate. This spec documents their logical content in prose; the wire
format is governed by the `.fbs` files, not JSON Schema.
...
Runs the `connect` login command against the daemon. On success the response body is a FlatBuffer
`LoginResult` and the three session headers are set.
...
Runs the `create` login command against the daemon. Identical to `/auth/connect` except the daemon
verb is `:do_login_command("create", …)`.
...
Round-trips to the daemon to verify the token is still valid. An ephemeral attach/detach is
performed; the connection is cleaned up automatically.
...
Sends a detach message to the daemon with `disconnected=true`, triggering `user_disconnected`.
Requires both the auth token and client credentials.
...
OAuth2 federated authentication. These routes are only registered when OAuth2 is enabled in the
server configuration; otherwise they return 404.

### `GET /v1/oauth2/config`

**Get OAuth2 configuration**

Returns whether OAuth2 is enabled and the list of configured providers. Route is only present when
OAuth2 is enabled.
...
Submits the user's account-choice decision (create new player or link to existing player). The
`oauth2_code` must resolve to a verified `Identity` on the server side. Requires `moor_oauth_nonce`
cookie.
...
Upgrades to a WebSocket connection for an already-authenticated player (the `connect` login path).

Auth credentials can be passed as HTTP headers or via `Sec-WebSocket-Protocol` subprotocols:
...
**Note:** Full WebSocket message framing is FlatBuffer-based and is outside the scope of this HTTP
spec.
...
Same as `/ws/attach/connect` but used when the player was created via `/auth/create`. See that
endpoint for auth details.
...
Sets a property to a new value specified as a MOO literal in the request body (e.g. `"hello"`, `42`,
`{1, 2, 3}`).
...
Resolves a CURIE (e.g. `moor:1`, `moor:system`) and returns object details.
...
Returns narrative events for the authenticated player. Exactly one of `since_seconds`,
`since_event`, or `until_event` must be provided.
...
Retrieves a property from `$system` by path (e.g. `server_version`). Auth token is optional —
...
Returns the set of features the server supports. Response is cached.
...
Returns 200 if the web host is running and has received a daemon ping recently; 503 otherwise.
