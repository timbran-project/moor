...
A mooR worker that performs filesystem operations on behalf of the server, confined to a single
sandbox directory. Like the curl worker, it attaches to the daemon over the worker RPC channel and
services requests dispatched from MOO code via `worker_request`.
