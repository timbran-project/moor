...
use moor_runtime_api::api::{
    self, ClientReply, ClientRequest, HostReply, HostRequest, InvocationMode, RuntimeClient,
};
use moor_runtime_api::{
    ...
    mk_resolve_msg, mk_retrieve_msg, mk_set_client_attribute_msg, mk_set_event_log_pubkey_msg,
    mk_update_property_msg, mk_verbs_msg, obj_fb, read_reply_result, rpc_message_error_from_ref,
    scheduler_error_from_ref, uuid_fb, verb_program_error_from_ref,
};
...
fn encode_client_request(
    ...
    Ok(match request {
        ...
        ClientRequest::Eval {
            ...
            InvocationMode::CaptureOutput { timeout } => {
                mk_eval_capture_msg(&auth_token, expression, timeout).ok_or_else(|| {
                    RpcError::CouldNotDecode("Failed to encode captured eval".to_string())
                })?
            }
        },
        ClientRequest::InvokeVerb {
            ...
            let msg = match mode {
                InvocationMode::Connected { client_token } => {
                    mk_invoke_verb_msg(&client_token, &auth_token, &object, &verb, args_refs)
                }
                InvocationMode::CaptureOutput { timeout } => {
                    mk_invoke_verb_capture_msg(&auth_token, &object, &verb, args_refs, timeout)
                }
            };
            msg.ok_or_else(|| {
...
mod tests {
    ...
    fn failed_verb_call_round_trip_preserves_output() {
        let output = NarrativeEvent::notify(
            v_obj(Obj::mk_id(7)),
        ...
        assert!(matches!(
            ...
            [event] if matches!(event.event(), Event::Notify { value, .. } if value.as_string() == Some("before failure"))
        ));
    }
}
