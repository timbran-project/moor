...
A web and desktop client for interacting with mooR worlds.
...
[Cowbell](https://github.com/timbran-project/cowbell/tree/moor-1.0-compatible) core.
...
This checkout contains the stable Meadow client for mooR 1.0. Development for later versions takes
place on `main`.

The schema and SDK are installed from this repository's npm workspace. They do not require an
...
# Start Meadow with the daemon and web host
npm run full:dev
...
Meadow uses the private `@moor/schema` and `@moor/web-sdk` workspaces from the same checkout. These
packages are not fetched from an external npm registry.

## Running Stable 1.0

Run `npm ci` and `npm run meadow:build` from the repository root, or use the Meadow assets bundled
with the 1.0 container image.
