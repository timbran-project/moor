...
class _CodeEditorState extends State<CodeEditor> {
      ...
      child = _CodeShortcuts(
        ...
        child: _CodeShortcutActions(
          ...
          child: Focus(
            ...
            includeSemantics: false,
            debugLabel: 'CodeEditor',
            // On web with ensureSemantics(), the SemanticsTextEditing strategy
            // can't route text input to custom DeltaTextInputClient widgets
            // that lack full semantic text field integration
            // (flutter/flutter#129324). Handle character input directly from
            // key events as a workaround.
            onKeyEvent: kIsWeb && !readOnly ? (node, event) {
              return _handleWebKeyEvent(event);
            } : null,
            child: detector
          )
        ),
  ...
  KeyEventResult _handleWebKeyEvent(KeyEvent event) {
    ...
    if (key == LogicalKeyboardKey.enter || key == LogicalKeyboardKey.numpadEnter) {
      _editingController.applyNewLine();
      _inputController.notifyListeners();
      return KeyEventResult.handled;
    }
    ...
    if (key == LogicalKeyboardKey.delete) {
      _editingController.deleteForward();
      _inputController.notifyListeners();
      return KeyEventResult.handled;
    }
    if (key == LogicalKeyboardKey.tab) {
      if (kb.isShiftPressed) {
        ...
        _editingController.applyIndent();
      }
      _inputController.notifyListeners();
      return KeyEventResult.handled;
    }
    ...
    if (char != null && char.isNotEmpty && char.codeUnitAt(0) >= 32) {
      _editingController.replaceSelection(char);
      _inputController.notifyListeners();
      return KeyEventResult.handled;
    }
    return KeyEventResult.ignored;
