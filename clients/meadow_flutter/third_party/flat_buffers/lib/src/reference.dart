...
class Reference {
  ...
  int _readInt(int offset, BitWidth width) {
    ...
    // dart2js does not support ByteData.getInt64/getUint64. Build from 32-bit
    // halves instead. This may lose precision above 2^53 on JS runtimes, but
    // avoids a hard runtime exception.
    final low = _buffer.getUint32(offset, Endian.little);
    final high = _buffer.getInt32(offset + 4, Endian.little);
  ...
  int _readUInt(int offset, BitWidth width) {
    ...
    // avoids a hard runtime exception.
    final low = _buffer.getUint32(offset, Endian.little);
    final high = _buffer.getUint32(offset + 4, Endian.little);
    return (high * 0x100000000) + low;
  }
