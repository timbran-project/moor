...
This deployment configuration provides a split-process mooR setup with HTTPS via Let's Encrypt
SSL/TLS certificates. It includes the web client, API server, telnet access, and automatic
...
1. **Copy this directory** to your deployment location:
   ```bash
   cp -r deploy/clustered/web-ssl /path/to/deployment
