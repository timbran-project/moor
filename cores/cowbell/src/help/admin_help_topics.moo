object ADMIN_HELP_TOPICS
  ...
  property topic_administration_sudo_grant (owner: ARCH_WIZARD, flags: "rc") = <HELP, .name = "@sudo-grant", .aliases = {"sudo-grant", "grant sudo", "sudo-grant"}, .content = "Usage: `@sudo-grant <player> as <wizard_player>`\n\nConfigures a player to execute commands as a specified wizard delegate via `@sudo`. If the player has no allowlist yet, a default seed is created.", .summary = "Grant sudo delegation", .category = 'administration, .see_also = {"@sudo-revoke", "@sudo-allow"}>;
  property topic_administration_sudo_log (owner: ARCH_WIZARD, flags: "rc") = <HELP, .name = "@sudo-log", .aliases = {"sudo-log", "sudo audit", "sudo-audit"}, .content = "Usage: `@sudo-log [N]`\n\nShows the most recent N sudo audit entries (default 20).", .summary = "Show sudo audit log", .category = 'administration, .see_also = {"@sudo-who"}>;
  property topic_administration_sudo_revoke (owner: ARCH_WIZARD, flags: "rc") = <HELP, .name = "@sudo-revoke", .aliases = {"sudo-revoke", "revoke sudo", "sudo-revoke"}, .content = "Usage: `@sudo-revoke <player>`\n\nRemoves sudo delegation, allowlist entries, and clears active sudo task markers for that player.", .summary = "Revoke sudo delegation", .category = 'administration, .see_also = {"@sudo-grant", "@sudo-allow"}>;
  property topic_administration_sudo_show (owner: ARCH_WIZARD, flags: "rc") = <HELP, .name = "@sudo-show", .aliases = {"sudo-show", "show sudo", "sudo-show"}, .content = "Usage: `@sudo-show <player>`\n\nShows delegate mapping, allowlist, and active sudo task entries for a player.", .summary = "Show sudo state for a player", .category = 'administration, .see_also = {"@sudo-who", "@sudo-allow"}>;
  property topic_administration_sudo_who (owner: ARCH_WIZARD, flags: "rc") = <HELP, .name = "@sudo-who", .aliases = {"sudo-who", "sudo active", "sudo-active"}, .content = "Usage: `@sudo-who`\n\nLists active sudo tasks and recent sudo audit log entries.", .summary = "Show active sudo and recent audit", .category = 'administration, .see_also = {"@sudo-show", "@sudo-log"}>;
