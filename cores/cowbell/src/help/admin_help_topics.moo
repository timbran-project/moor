object ADMIN_HELP_TOPICS
  ...
  property topic_administration_sudo_grant (owner: ARCH_WIZARD, flags: "rc") = <HELP, .content = "Usage: `@sudo-grant <player> as <wizard_player>`\n\nConfigures a player to execute commands as a specified wizard delegate via `@sudo`. If the player has no allowlist yet, a default seed is created.", .name = "@sudo-grant", .aliases = {"sudo-grant", "grant sudo", "sudo-grant"}, .category = 'administration, .summary = "Grant sudo delegation", .see_also = {"@sudo-revoke", "@sudo-allow"}>;
  property topic_administration_sudo_log (owner: ARCH_WIZARD, flags: "rc") = <HELP, .content = "Usage: `@sudo-log [N]`\n\nShows the most recent N sudo audit entries (default 20).", .name = "@sudo-log", .aliases = {"sudo-log", "sudo audit", "sudo-audit"}, .category = 'administration, .summary = "Show sudo audit log", .see_also = {"@sudo-who"}>;
  property topic_administration_sudo_revoke (owner: ARCH_WIZARD, flags: "rc") = <HELP, .content = "Usage: `@sudo-revoke <player>`\n\nRemoves sudo delegation, allowlist entries, and clears active sudo task markers for that player.", .name = "@sudo-revoke", .aliases = {"sudo-revoke", "revoke sudo", "sudo-revoke"}, .category = 'administration, .summary = "Revoke sudo delegation", .see_also = {"@sudo-grant", "@sudo-allow"}>;
