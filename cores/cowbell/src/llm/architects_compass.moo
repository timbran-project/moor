object ARCHITECTS_COMPASS
  ...
  verb _setup_agent (this none this) owner: ARCH_WIZARD flags: "rxd"
    ...
    set_passage_description_tool = $llm_agent_tool:mk("set_passage_description", "Set the narrative description for a passage/exit. IMPORTANT: Passages are flyweights stored in areas, NOT properties on rooms. Do NOT try to use room message templates or properties like 'down_msg' - they don't exist for passages. This tool is THE way to set exit descriptions. The description appears from the source_room's perspective (what you see looking at the exit). When ambient=true (default), the description integrates into the room's look output.", ["type" -> "object", "properties" -> ["direction" -> ["type" -> "string", "description" -> "Direction/exit label (e.g. 'north', 'up', 'down')"], "description" -> ["type" -> "string", "description" -> "Narrative description for the passage (e.g. 'A narrow service door leads down to worn stairs below.')"], "source_room" -> ["type" -> "string", "description" -> "Source room (optional, defaults to wearer's current location)"], "ambient" -> ["type" -> "boolean", "description" -> "If true, description integrates into room description. If false, shows in exits list (default: true)"]], "required" -> {"direction", "description"}], this, "_tool_set_passage_description");
    ...
    area_map_tool = $llm_agent_tool:mk("area_map", "Get a graph of all rooms in the current area with their passage connections. Shows each room and where each exit leads (e.g., 'Lobby -> down -> Service Stairs'). Use this to understand the spatial connectivity before modifying passages.", ["type" -> "object", "properties" -> [], "required" -> {}], this, "_tool_area_map");
  ...
  verb _tool_remove_passage (this none this) owner: ARCH_WIZARD flags: "rxd"
    ...
    passage == false && return "No passage found between " + tostr(source_room) + " and " + tostr(target_room) + ".";
  ...
  verb _tool_set_passage_description (this none this) owner: ARCH_WIZARD flags: "rxd"
    ...
    target_passage == false && return "No passage found in direction '" + direction + "' from " + tostr(source_room) + ".";
    from_cap = wearer:find_capability_for(source_room, 'room);
    from_target = typeof(from_cap) == FLYWEIGHT ? from_cap | source_room;
    try
      from_target:check_can_dig_from();
  ...
  verb _tool_area_map (this none this) owner: ARCH_WIZARD flags: "rxd"
    "Tool: Get list of all rooms in the current area with passage connections";
    {args_map} = args;
    wearer = this:_action_perms_check();
    current_room = wearer.location;
    ...
    result = {"Current Location: " + `current_room:name() ! ANY => tostr(current_room)' + " (" + tostr(current_room) + ")", "Area: " + `area:name() ! ANY => tostr(area)' + " (" + tostr(area) + ")", "", "Rooms and Connections:"};
    for o in (area.contents)
      if (valid(o) && isa(o, $room))
        room_line = "  * " + `o:name() ! ANY => tostr(o)' + " (" + tostr(o) + ")" + (o == current_room ? " (you are here)" | "");
        result = {@result, room_line};
        "Show exits from this room with destinations";
        passages = `area:passages_from(o) ! ANY => {}';
        for p in (passages)
          label = `p:label_for(o) ! ANY => "?"';
          dest = `p:other_room(o) ! ANY => #-1';
          dest_name = valid(dest) ? `dest:name() ! ANY => tostr(dest)' | "unknown";
          result = {@result, "      -> " + label + " -> " + dest_name + " (" + tostr(dest) + ")"};
        endfor
      endif
    endfor
    return result:join("\n");
