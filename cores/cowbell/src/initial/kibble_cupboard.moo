object KIBBLE_CUPBOARD
  ...
  property kibble_taken_reaction (owner: HACKER, flags: "r") = <REACTION, .enabled = true, .when = <RULE, .name = 'kibble_check, .body = {{'isa, 'Item, CAT_KIBBLE}}, .variables = {'Item}, .head = 'kibble_check>, .trigger = 'on_take, .effects = {{'trigger, HENRI, 'on_kibble_taken}}, .fired_at = 0>;
  property waft_reaction (owner: HACKER, flags: "r") = <REACTION, .enabled = true, .when = 0, .trigger = 'on_open, .effects = {
