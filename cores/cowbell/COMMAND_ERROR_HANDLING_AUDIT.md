...
- `$player` has broad `except ANY` blocks — most are proper command-boundary catches with error reporting, but verification is ongoing. `examine` object matching was narrowed so expected not-found remains not-found while unexpected matcher failures are reported.
