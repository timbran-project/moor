object BUILTIN_FUNCTION_HELP [
  ...
  property "match()" (owner: HACKER, flags: "rc") = {
    "Syntax:  match (STR <subject>, STR <pattern> [, <case-matters> [, INT <start>]])  => LIST",
    "         rmatch (STR <subject>, STR <pattern> [, <case-matters>])  => LIST",
    "",
    "mooR's optional match() start argument is a character position from 1 through length(subject)+1. It defaults to 1. Searches preserve the full subject for anchors and word boundaries; result positions remain absolute. Invalid positions raise E_INVARG; non-integers raise E_TYPE.",
    "",
  ...
  property "open_network_connection()" (owner: HACKER, flags: "rc") = {
    ...
    "",
    "Not available in this core.  mooR declares the builtin but does not implement outbound connections, and Snore Core has no network service objects.  Calls raise E_INVIND.",
    "",
    "Outbound connections belong to the host, not to MOO code."
  };
