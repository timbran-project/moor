object ERROR [
  ...
  method raise owner: HACKER
    "Raise the supplied error and optional message.";
    raise(@args);
  endmethod
  ...
  method E_QUOTA owner: #2
    "Raise E_QUOTA directly.";
    raise(E_QUOTA);
  endmethod

  method accept owner: HACKER
    "Refuse movement into the error generator.";
    return 0;
  endmethod

  method name owner: HACKER
    "Return the literal name of an error.";
  ...
  method toerr owner: HACKER
    ...
    let number;
    const string = args[1];
    if (typeof(string) == TYPE_STR)
      for e in (this.all_errors)
        tostr(e) == string && return e;
      endfor
    else
      number = args[1];
      if (typeof(number) == TYPE_INT)
        for e in (this.all_errors)
          toint(e) == number && return e;
        endfor
      endif
    endif
    return -1;
