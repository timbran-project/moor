object LETTER [
  ...
  verb burn (this none none) owner: #2 flags: "rd"
    ...
    const msg = result ? this:oburn_succeeded_msg() | this:oburn_failed_msg();
    if (msg)
  ...
  method "burn_succeeded_msg oburn_succeeded_msg burn_failed_msg oburn_failed_msg" owner: #2
    "Expand the corresponding letter-burning message.";
    let msg;
    return (msg = this.(verb)) ? $string_utils:pronoun_sub(msg) | "";
  endmethod

  method do_burn owner: #2
    "Schedule recycling of a controlled letter; refuse to recycle the prototype.";
