object BUILDER_HELP [
  ...
  property "@audit" (owner: HACKER, flags: "r") = {
    ...
    "It is also possible to restrict the range of object numbers to include only those above a given number (`from') or below a given number (`to'). Ranges apply to numbered objects; user objects created with UUID identifiers are always included.",
    "",
  ...
  property "@build-options" (owner: HACKER, flags: "rc") = {
    ...
    "",
    "Builder flags control audit output:",
    " +audit_bytes   Show byte counts for objects smaller than 1K.",
    " +audit_float   Show fractional sizes in K.",
    "",
    "Creation and recycling use $recycler. User objects receive UUID identifiers; objects are not pooled for reuse."
  };
