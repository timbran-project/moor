object WIZ_HELP [
  ...
  property "$site_db" (owner: HACKER, flags: "rc") = {
    ...
    "",
    "The index is a lowercase-keyed map from domain to the players and subdomains below it."
  };
  ...
  property "@make-guest" (owner: HACKER, flags: "rc") = {
    ...
    "",
    "The core ships with $default_guest, a player-flagged child of $guest named Guest with the guest alias, so `connect guest' works out of the box.  @make-guest adds more guest characters.",
    "",
  ...
  property "@make-player" (owner: HACKER, flags: "rc") = {
    ...
    "",
    "The generated password is displayed to the wizard."
  };
  ...
  property "recycling-players" (owner: HACKER, flags: "rc") = {
    ...
    "  @recycle em",
    "",
    "A wizard can rebuild player-class .owned_objects lists with $wiz_utils:initialize_owned(). It uses the database ownership index and includes UUID objects. Each owner's list is repaired before any suspension."
  };
  property redlist (owner: HACKER, flags: "rc") = {"*forward*", "blacklist"};
