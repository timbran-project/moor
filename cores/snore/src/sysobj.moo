object SYSOBJ [
  ...
  property default_guest (owner: #2, flags: "rc") = DEFAULT_GUEST;
  property default_player (owner: #2, flags: "rc") = DEFAULT_PLAYER;
  property default_player_help (owner: #2, flags: "rc") = DEFAULT_PLAYER_HELP;
  ...
  property player_class (owner: #2, flags: "rc") = DEFAULT_PLAYER;
  ...
  property programmer_feature (owner: #2, flags: "r") = PROGRAMMER_FEATURE;
  ...
  property utility_feature (owner: #2, flags: "rc") = UTILITY_FEATURE;
  ...
  property wizard_feature (owner: #2, flags: "r") = WIZARD_FEATURE;
  ...
  method do_command owner: #2
    "Handle server commands with scoped object matching; return false for the retained huh path.";
    callers() && !caller_perms().wizard && raise(E_PERM);
    !args && return true;
    const command = argstr;
    set_task_perms(player);
    const options = ['complex -> true, 'fuzzy_threshold -> $server_options.command_fuzzy_threshold];
    const environment = player:match_environment(command, options);
    return this:_command_handler(command, environment, options);
  endmethod

  method _command_handler owner: #2
    "Select the first viable object/verb combination. Matching and dispatch have no intervening commit.";
    caller == this || raise(E_PERM);
    const {command, environment, options} = args;
    set_task_perms(player);
    const parsed = parse_command(command, environment, options['complex], options['fuzzy_threshold]);
    const symbolic_keys = maphaskey(parsed, 'verb);
    const key = {name} => symbolic_keys ? tosym(name) | name;
    const command_environment = player:command_environment();
    const direct = parsed[key("dobj")] == $ambiguous_match ? parsed[key("ambiguous_dobj")] | {parsed[key("dobj")]};
    const indirect = parsed[key("iobj")] == $ambiguous_match ? parsed[key("ambiguous_iobj")] | {parsed[key("iobj")]};
    for direct_object in (direct)
      for indirect_object in (indirect)
        let candidate = parsed;
        candidate[key("dobj")] = direct_object;
        candidate[key("iobj")] = indirect_object;
        for match in (find_command_verb(candidate, command_environment))
          const {target, specification} = match;
          if (target in player.features)
            const direct_specs = candidate[key("dobjstr")] ? {"any"} | {"none", "any"};
            const indirect_specs = candidate[key("iobjstr")] ? {"any"} | {"none", "any"};
            const prepositions = {"any", candidate[key("prepstr")] ? $code_utils:full_prep(candidate[key("prepstr")]) | "none"};
            const feature_command = target:has_feature_verb(candidate[key("verb")], direct_specs, prepositions, indirect_specs);
            if (!feature_command || feature_command != candidate[key("verb")])
              continue;
            endif
          endif
          this:_dispatch_command(target, specification[4], candidate);
          return true;
        endfor
      endfor
    endfor
    "The native fallback preserves room huh overrides, exits, editor shortcuts, and feature redirects.";
    return false;
  endmethod

  method _dispatch_command owner: #2
    "Dispatch a selection made by the system handler with player authority and one builtin grant.";
    caller == this || raise(E_PERM);
    const {target, command, parsed} = args;
    set_task_perms(player, {{"builtin_call", "dispatch_command_verb"}});
    return dispatch_command_verb(target, command, parsed);
  endmethod
  ...
  method do_login_command owner: #2
    ...
    const host = $string_utils:connection_hostname(connection_name(player));
  ...
  method server_started owner: #2
    "Record startup time and initialize login services. Server invocation only.";
    if (!callers())
  ...
  method "core_object_info core_objects" owner: #2
    "Traverse declared scalar core links with caller authority; return objects or extraction metadata.";
    "Hooks select proxies and additional properties. A failed hook stops selection before deletion.";
    const {?variant = {}, ?extracting = false} = args;
    if (extracting)
      const frame = callers()[1];
      frame[2] == "make-core-database" && frame[3].wizard && frame[4] == $wizard_feature || raise(E_PERM);
    endif
    ...
    let proxies = {};
    let references = {};
    let pending = {{#0, {"sysobj", "owner"}}};
    while (pending)
      const {holder, properties} = pending[1];
      pending = pending[2..$];
      let followed = {};
      for property_name in (properties)
        const property = tostr(property_name);
        let object = holder.(property);
        if (typeof(object) != TYPE_OBJ || !valid(object))
          continue;
        endif
        followed = {@followed, property};
        if (object in originals || object in selected)
          continue;
        endif
        if ($object_utils:has_callable_verb(object, "proxy_for_core"))
          const replacement = object:proxy_for_core(variant, extracting);
          typeof(replacement) == TYPE_OBJ || raise(E_TYPE, "Core proxy must be an object.");
          if (replacement != object)
            !(replacement in originals) || raise(E_RECMOVE, "Core proxy loop.");
            originals = {@originals, object};
            proxies = {@proxies, replacement};
            if (!valid(replacement) || replacement in selected)
              continue;
            endif
            object = replacement;
          endif
        endif
        if ($object_utils:has_callable_verb(object, "include_for_core"))
          const included = object:include_for_core(variant);
          typeof(included) == TYPE_LIST || raise(E_TYPE, "Core inclusion must return a property list.");
          included && (pending = {@pending, {object, included}});
        endif
        selected = setadd(selected, object);
      endfor
      references = {@references, {holder, followed}};
    endwhile
    let skipped = {};
    for object in (selected)
      let ancestor = parent(object);
      while (valid(ancestor))
        !(ancestor in selected) && (skipped = setadd(skipped, ancestor));
        ancestor = parent(ancestor);
      endwhile
    endfor
    if (verb == "core_object_info")
      return {selected, references, skipped, originals, proxies};
    endif
    return selected;
  endmethod
  ...
  method init_for_core owner: #2
    ...
    if (caller_perms().wizard)
      ...
      $core_history = {{$mail_agent.moo_name, server_version(), time()}, @$core_history};
      $shutdown_message = "";
      $shutdown_time = 0;
      $dump_interval = 3600;
  ...
  method "user_disconnected user_client_disconnected" owner: #2
    "Run player and location disconnect hooks. Server invocation only.";
    callers() && return;
    if (args[1] < #0)
  ...
  method "bf_add_property add_property" owner: #2
    ...
    let retval;
    const who = caller_perms();
    const {what, propname, value, info} = args;
    if (typeof(what) != TYPE_OBJ)
      retval = E_TYPE;
    elseif (!valid(what))
      retval = E_INVARG;
  ...
  method "bf_recycle recycle" owner: #2
    "Recycle a controlled object, update quota and ownership records, then schedule task cleanup.";
    "The native builtin is called here so it does not invoke its system override again.";
    const {item} = args;
    const actor = caller_perms();
    valid(item) || raise(E_INVARG);
    $perm_utils:controls(actor, item) || raise(E_PERM);
    if (is_player(item))
      actor.wizard || raise(E_PERM);
      $wiz_utils:unset_player(item);
    endif
    const owner = item.owner;
    const size = $object_utils:has_property(item, "object_size") ? item.object_size[1] | -1;
    const result = `recycle(item) ! ANY';
    if (typeof(result) == TYPE_ERR)
      return $code_utils:dflag_on() ? raise(result) | result;
    endif
    if (valid(owner))
      if ($object_utils:isa(owner, $player))
        $quota_utils:reimburse_recycled_object(owner, size);
      endif
      const owned = `owner.owned_objects ! E_PROPNF => false';
      if (typeof(owned) == TYPE_LIST && !is_clear_property(owner, "owned_objects"))
        owner.owned_objects = setremove(owned, item);
      endif
    endif
    "Task cleanup forks only after the object, quota, and ownership changes are complete.";
    $recycler:kill_all_tasks(item);
    return result;
  endmethod
  ...
  method "bf_set_verb_info set_verb_info" owner: #2
    ...
    let no;
    const {o, v, i} = args;
    const vi = `verb_info(o, v) ! ANY';
    if (typeof(vi) == TYPE_ERR)
      "probably verb doesn't exist";
      retval = vi;
    else
      cp = caller_perms();
      if (!$perm_utils:controls(cp, vi[1]))
        "perms don't control the current verb owner";
        retval = E_PERM;
      else
        no = typeof(i) == TYPE_LIST ? i[1] | $nothing;
        if (typeof(i) != TYPE_LIST || typeof(no) != TYPE_OBJ)
          "info is malformed";
          retval = E_TYPE;
  ...
  method "bf_match match" owner: #2
    "Call native match and preserve the caller's debug-error behavior.";
    const m = `match(@args) ! ANY';
    return typeof(m) == TYPE_ERR && $code_utils:dflag_on() ? raise(m) | m;
  endmethod

  method "bf_rmatch rmatch" owner: #2
    "Call native rmatch and preserve the caller's debug-error behavior.";
    const r = `rmatch(@args) ! ANY';
  ...
  method handle_uncaught_error owner: #2
    "Deliver or mail an uncaught-error traceback and invoke the player's error hook.";
    if (!callers())
      const {code, msg, value, stack, traceback} = args;
  ...
  method checkpoint_started owner: #2
    "Record checkpoint progress and notify the optional local service. Server invocation only.";
    callers() && raise(E_PERM);
    $login.checkpoint_in_progress = true;
  ...
  method bf_force_input owner: #2
    ...
    let retval;
    const {conn, line, ?at_front = 0} = args;
  ...
  method "bf_set_property_info set_property_info" owner: #2
    "Validate protected property changes and apply metadata with caller authority.";
    let what;
    let propname;
    let info;
    let owner;
  ...
  method include_for_core owner: #2
    "Include system property references during core extraction.";
    return properties(this);
  endmethod

  method handle_task_timeout owner: #2
    "Deliver or mail a task-timeout traceback and invoke the player's timeout hook.";
    if (!callers())
      const {resource, stack, traceback} = args;
      if (!$object_utils:connected(player))
        "Mail the player the traceback if e isn't connected.";
        $mail_agent:send_message(#0, player, {"traceback", $gripe_recipients}, traceback);
      endif
