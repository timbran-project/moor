object ROOT_CLASS [
  ...
  method initialize owner: #2
    "Record ownership and initialize object metadata when the caller controls this object.";
  ...
  method recycle owner: #2
    "Authorize the cleanup hook. A direct call does not delete the object or change its accounting.";
    "The system recycle wrapper removes ownership entries after successful deletion.";
  ...
  method set_name owner: #2
    ...
    let e;
    !caller_perms().wizard && (is_player(this) || (caller_perms() != this.owner && this != caller)) && return E_PERM;
  ...
  method titlec owner: #2
    "Return a capitalized title, using namec when provided.";
  ...
  method set_aliases owner: #2
    ...
    const aliases = args[1];
    typeof(aliases) != TYPE_LIST && return E_TYPE;
    for s in (aliases)
      typeof(s) != TYPE_STR && return E_INVARG;
    endfor
  ...
  method match owner: #2
    "Match a name or alias among this object's contents.";
  ...
  method look_self owner: #2
    "Show the description, or a default message when it is empty.";
  ...
  method notify owner: #2
    "Deliver output when this object is a player.";
    if (is_player(this))
      notify(this, @args);
    endif
  endmethod
  ...
  method tell owner: #2
    "Concatenate values and send them through this object's notification interface.";
  ...
  method accept owner: #2
    "Check acceptance with caller authority.";
    set_task_perms(caller_perms());
    return this:acceptable(@args);
  endmethod

  method moveto owner: #2
    "Move this object with its owner's authority, returning any movement error.";
  ...
  method "eject eject_nice eject_basic" owner: #2
    ...
    let where = nice && $object_utils:has_property(what, "home") ? what.home | false;
    if (nice && typeof(where) == TYPE_OBJ && where != this && (is_player(what) ? `where:accept_for_abode(what) ! ANY' | `where:acceptable(what) ! ANY'))
    else
  ...
  method is_unlocked_for owner: #2
    "Return whether the supplied principal satisfies this object's key.";
    return this.key == 0 || $lock_utils:eval_key(this.key, args[1]);
  endmethod

  method huh owner: #2
    "Handle an unmatched command with the invoking player's authority.";
  ...
  method examine_verbs owner: #2
    ...
    const name = dobjstr;
    let vrbs = {};
    const commands_ok = `this:examine_commands_ok(who) ! ANY => 0';
    const dull_classes = {$root_class, $room, $player, $prog, $builder};
    let what = this;
    const hidden_verbs = this:hidden_verbs(who);
    while (what != $nothing)
  ...
  method get_message owner: #2
    ...
    let msg;
    !(caller == this || $perm_utils:controls(caller_perms(), this)) && return E_PERM;
    const t = typeof(msg = `this.(args[1] + "_msg") ! ANY');
    t in {TYPE_ERR, TYPE_STR} || (t == TYPE_LIST && msg && typeof(msg[1]) == TYPE_STR) && return msg;
    return {1, msg};
  endmethod

  method "room_announce*_all_but" owner: #2
    "Forward an announcement to the location, ignoring a missing or failing output hook.";
    try
  ...
  method examine_verb_ok owner: #2
    ...
    if (caller == this || $perm_utils:controls(caller_perms(), this))
      const {loc, index, info, syntax, commands_ok, hidden_verbs} = args;
      const vname = info[3];
  ...
  method "announce*_all_but" owner: #2
    "Provide an empty announcement hook for non-room objects.";
    return;
  endmethod
  ...
  method acceptable owner: #2
    "Refuse movement by default; descendants supply an acceptance policy.";
    return false;
