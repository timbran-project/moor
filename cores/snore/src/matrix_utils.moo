object MATRIX_UTILS [
  ...
  method "vector_add vector_sub vector_mul vector_div" owner: HACKER
    ...
    !args && return raise(E_INVARG);
    const type = verb[$ - 2..$];
    const lresult = max = length(args[1]);
    let results = args[1];
  ...
  method transpose owner: HACKER
    ...
    const {mat} = args;
    !this:is_matrix(mat) && return raise("E_INVMAT", "Invalid Matrix Format");
    const j = this:dimensions(mat)[2];
    let result = {};
    for n in [1..j]
  ...
  method determinant owner: HACKER
    ...
    "";
    const {mat} = args;
    !this:is_square(mat) && return raise("E_INVMAT", "Invalid Matrix Format");
    if (this:dimensions(mat) == {1, 1})
      return mat[1][1];
    elseif (this:dimensions(mat)[1] == 2)
      return mat[1][1] * mat[2][2] - mat[1][2] * mat[2][1];
    else
      let result = typeof(mat[1][1]) == TYPE_INT ? 0 | 0.0;
      let coeff = typeof(mat[1][1]) == TYPE_INT ? 1 | 1.0;
  ...
  method inverse owner: HACKER
    ...
    const det = this:determinant(mat);
    tofloat(det) == 0.0 && return raise("E_NOINV", "No Inverse Exists");
    let result = {};
    for k in [1..i]
      let sub = {};
      for l in [1..j]
  ...
  method is_null owner: HACKER
    ":is_null(M) => 1 iff M is O.";
    let mat;
    const m = length(mat = args[1]);
    !this:is_square(mat) && return 0;
    for i in [1..m]
      for j in [1..m]
        mat[i][j] != 0 && return 0;
      endfor
  ...
  method is_identity owner: HACKER
    ...
    for i in [1..m]
      for j in [1..m]
        mat[i][j] != 0 && (i != j ? 1 | mat[i][j] != 1) && return 0;
      endfor
    endfor
    return 1;
  endmethod
  ...
  method "cross_prod outer_prod vector_prod" owner: HACKER
    ...
    const {v1, v2} = args;
    const l = length(v1);
    if (l != length(v2) || l != 3 || !this:is_vector(v1) || !this:is_vector(v2))
      return raise("E_INVVEC", "Invalid Vector Format");
    endif
    const mat = {{1, 1, 1}, v1, v2};
    let coeff = 1;
    let result = {};
    for n in [1..3]
  ...
  method "dot_prod inner_prod scalar_prod" owner: HACKER
    ...
    const {v1, v2} = args;
    const l = length(v1);
    if (l != length(v2) || !this:is_vector(v1) || !this:is_vector(v2))
      return raise("E_INVVEC", "Invalid Vector Format");
    endif
    const temp = this:vector_mul(v1, v2);
    let result = typeof(temp[1]) == TYPE_INT ? 0 | 0.0;
    for n in [1..l]
      $command_utils:suspend_if_needed(0);
  ...
  method "scalar_vector_add scalar_vector_sub scalar_vector_mul scalar_vector_div" owner: HACKER
    ...
    !this:is_vector(vval) && return raise("E_INVVEC", "Invalid Vector Format");
    const type = verb[$ - 2..$];
  ...
  method "scalar_matrix_mul scalar_matrix_div" owner: HACKER
    ...
    !this:is_matrix(mval) && return raise("E_INVMAT", "Invalid Matrix Format");
    let results = {};
  ...
  method is_matrix owner: HACKER
    ...
    let m;
    {m} = args;
    typeof(m) != TYPE_LIST || typeof(m[1]) != TYPE_LIST && return 0;
    const len = length(m[1]);
    for v in (m)
      !this:is_vector(v) || length(v) != len && return 0;
    endfor
    return 1;
  endmethod
  ...
  method is_vector owner: HACKER
    ...
    let flag = 1;
    {v} = args;
    typeof(v) != TYPE_LIST && return 0;
    for n in (v)
      const ntype = typeof(n);
      if (ntype != TYPE_INT && ntype != TYPE_FLOAT)
        flag = 0;
        break;
      endif
      $command_utils:suspend_if_needed(0);
  ...
  method "is_reflexive is_areflexive" owner: HACKER
    ...
    let bad;
    const {m} = args;
    !this:is_square(m) && return raise("E_INVMAT", "Invalid Matrix Format");
    let good = bad = 0;
    for n in [1..length(m)]
      if (!m[n][n])
        bad = 1;
