object HELP [
  ...
  property "@version" (owner: HACKER, flags: "r") = {
    "Syntax:  @version",
    "",
    "Prints the mooR server version and identifies Snore Core."
  };
  ...
  property features (owner: HACKER, flags: "rc") = {
    ...
    "Role command packs require their supporting player class. See `help player-classes'."
  };
  ...
  property introduction (owner: HACKER, flags: "r") = {
    "Welcome to Snore Core -- just boring enough. This LambdaCore fork for mooR keeps familiar MOO commands and live programming.",
    "",
    "Explore rooms, talk to other players, try the objects they build, and create places and things of your own. Type help index for topics.",
    "",
  ...
  property news (owner: HACKER, flags: "rc") = {
    ...
    "        news archive",
    "",
    "Read the latest edition of the %[$mail_agent.moo_name] Newspaper, which carries articles concerning recent changes to the MOO server or to the main public classes, or other articles of interest to the MOO at large.",
    "",
  ...
  property "player-classes" (owner: HACKER, flags: "rc") = {
    "$player supplies shared character and session behavior. $mail_recipient_class adds mail, news, refusals, and spurns.",
    "$default_player adds non-VR conveniences and is the initial $player_class for new accounts.",
    "$builder, $prog, and $wiz inherit in that order from $default_player.",
    "Their separate Builder, Programmer, and Wizard Features are installed by default on the matching classes.",
    "The classes supply the methods and state used by those command packs.",
    "Installing a pack does not grant programmer or wizard permission.",
    "@programmer moves a player into $prog when needed. Existing $prog descendants keep their class.",
    "A game class can derive from $mail_recipient_class without inheriting the default player's utilities."
  };
  ...
  property tinymud (owner: HACKER, flags: "r") = {
    "This is yduJ's table of tinymud commands and their equivalents in LambdaMOO.  All the commands mentioned here have help nodes on LambdaMOO.",
    "",
  ...
  method player_quota owner: #2
    "Return the base player's object-quota allowance.";
    return $player.ownership_quota;
  endmethod

  method prog_quota owner: #2
    "Return the programmer prototype's object-quota allowance.";
    return $prog.ownership_quota;
  endmethod

  method get_topic owner: #2
    "Return stored help or an object's help message, with a diagnostic when absent.";
    let what;
    const text = pass(@args);
    const object = $string_utils:match_object(what = args[1], player.location);
    text != E_PROPNF || !valid(object) && return text;
    const ohelp = `object:help_msg() ! ANY' || `object.help_msg ! ANY';
    ohelp && return {tostr(object.name, " (", object, "):"), "----", @typeof(ohelp) == TYPE_LIST ? ohelp | {ohelp}};
    const about = $object_utils:has_verb(object, "about");
  ...
  method find_topics owner: #2
    "Find stored help topics or a matching object's help entry.";
    let what;
    const topiclist = pass(@args);
    topiclist || !args && return topiclist;
    const o = $string_utils:match_object(what = args[1], player.location);
    valid(o) && return {what};
    return {};
  endmethod

  method full_index owner: HACKER
    "Collect topic indexes from all configured help databases.";
    let text = {};
  ...
  method index_list owner: HACKER
    "Return the names and descriptions of available help indexes.";
    const hdr = "Available Help Indices";
    let text = {"", hdr, $string_utils:space(hdr, "-")};
    for db in ($code_utils:help_db_list())
      try
    ...
    const full = this:find_full_index_topic();
    if (full)
  ...
  method wizard_list owner: #2
    "Return the advertised wizards and their public identities.";
    let wpi;
    let wizzes = {};
    ...
    let numwiz = length(wizzes);
    const hlist = {"ArchWizard:", "Wizard" + (numwiz == 2 ? ":" | "s:"), @$list_utils:make(max(0, numwiz - 2), "")};
    let slist = {};
  ...
  method dump_topic owner: HACKER
    "Return commands that recreate a stored topic or an object's help text.";
    const text = pass(@args);
    text != E_PROPNF && return text;
    const object = $string_utils:match_object(args[1], player.location);
    !valid(object) || !$object_utils:has_property(object, "help_msg") && return text;
    const help = object.help_msg;
    return {tostr(";;", $code_utils:corify_object(object), ".help_msg = $command_utils:read_lines()"), @$command_utils:dump_lines(typeof(help) == TYPE_LIST ? help | {help})};
  endmethod
  ...
  method find_full_index_topic owner: HACKER
    ...
    for p in (`properties(this) ! E_PERM => {}')
      `this.(p)[1] ! ANY' == "*full_index*" && return p;
    endfor
    return 0;
  endmethod
endobject
