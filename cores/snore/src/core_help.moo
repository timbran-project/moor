object CORE_HELP [
  ...
  property "$container" (owner: HACKER, flags: "rc") = {
    ...
    "",
    ".opened is true when the container is open",
    ".dark is true when the contents are hidden",
    ".opaque controls how opening the container affects visibility",
    "   == 0  contents are always visible",
    "   == 1  contents are visible only when the container is open",
    "   == 2  contents are always hidden",
    "",
    ":set_opaque(newvalue)   ",
    "  clamps an integer to 0..2, then updates .opaque and .dark",
    "  returns the clamped integer, or E_PERM or E_INVARG",
    "",
    ":set_opened(newvalue)   ",
    "  opens or closes the container, updating .opened and .dark together",
    "  returns the boolean opening state, or E_PERM",
    "",
  ...
  property "$login" (owner: HACKER, flags: "rc") = {
    ...
    "  req*uest @req*uest -- request a character by email address",
    "  oauth2_*           -- OAuth2 login hooks used by external hosts",
  ...
  property "$mail_recipient" (owner: HACKER, flags: "rc") = {
    ...
    "  MR-expiration   -- expiring messages from recipients"
  };
  ...
  property "$player_db" (owner: HACKER, flags: "rc") = {
    ...
    "Database of Players",
    "-------------------",
    "This is the player-name index.  It holds the {name/alias,#objectid} pairs for every",
    ...
    "",
    "The internal representation is a lowercase-keyed map from name or alias to player.",
    "",
  ...
  property "$recycler" (owner: #2, flags: "r") = {
    ...
    "=========",
    "",
    "The recycling center is the core's creation and recycling interface. It keeps quota and ownership bookkeeping consistent for user objects.",
    "",
    "Most Useful Verbs",
    "-----------------",
    "",
    "$recycler:_recycle( object )",
    "  Recycles the object after checking that the caller controls it. It handles .ownership_quota and .owned_objects properly.  Generally, use this instead of a recycle() in your verbs.",
    "",
    ...
    "  This is a variant of the server built-in valid() that also returns false for children of $garbage."
  };
  ...
  property "MR-expiration" (owner: HACKER, flags: "rc") = {
    ...
    "Players:",
    "",
    "  Players may completely turn off mail expiration by setting the mail option `expire' off. If it is on, then $mail_agent.player_expire_time is the expire period used for mail on players. Expired messages are deleted. See `help mail-options'.",
    "",
    "Mail Recipients:",
    "",
    ...
    "",
    "  Similar to players, the owner of a mail recipient may set an expiration period for messages on the recipient. The command is:",
    "",
    "    @register <recipient> to <address>",
    "",
    "If the owner leaves off the <address>, the current registered address (if any) and expiration period will be returned. Network mail is not available, so registration is limited to recording the owner's registered address."
  };
  ...
  property init_for_core (owner: HACKER, flags: "rc") = {
    ...
    "",
    "This verb is called in the final stage of core extraction (see help make-core-database), which occurs after all non-core objects have been recycled, selected UUID objects have received numbered IDs, and all retained objects have moved to #-1.  This verbcall then performs any final cleanups to establish the initial state of the object and is (pretty much) the last thing to happen to the object before the new core database is saved.",
    "",
    ...
    "",
    "(3) Existing numbered IDs remain unchanged. Declared scalar core links are repaired when selected UUID objects receive numbers. Hooks must repair or discard other references, including IDs in lists, maps, and verb code.",
    "",
  ...
  property "mail-system" (owner: HACKER, flags: "rc") = {
    "Mail System",
    "-----------",
    "The following topics describe the guts of the Snore Core mail system",
    "",
  ...
  property "make-core-database" (owner: HACKER, flags: "rc") = {
    "Syntax: make-core-database [variant]",
    "        make-core-database resume",
    "",
    "This command destroys world data. Run it only in an isolated disposable copy of the database.",
    "Connect as the numbered core wizard, normally #2. Disconnect everyone else and prevent outside connections.",
    "The command asks for confirmation twice. It records the selected objects and its progress before cleanup.",
    "",
    "Core selection follows $core_objects(), include_for_core, and proxy_for_core. A failed selection hook stops extraction.",
    "Unsaved objects are removed, including UUID objects. Their recycle and exitfunc definitions are removed first.",
    "Existing numbered core objects keep their IDs. Selected UUID objects receive new numbered IDs.",
    "Numbering can contain gaps. The allocation high-water mark is not reset.",
    "Declared scalar references and property/verb ownership are repaired after renumbering.",
    "Object references inside lists, maps, or verb code are not rewritten. init_for_core hooks must repair or discard that data.",
    "Parents initialize before children. Ownership lists and byte-quota measurements are rebuilt.",
    "After successful extraction, the server shuts down. Its native database contains the extracted world.",
    "Restart that isolated copy to inspect it. Use @dump-database to export objdef when an export directory is configured.",
    "",
    "If a task stops, use make-core-database resume as the same wizard. Do not start another extraction.",
    "Progress is stored in $wizard_feature.__mcd__state. A live extraction task prevents another worker from resuming.",
    "Resume continues at the recorded phase and object. Completed transactions are not rolled back.",
    "Custom hooks can suspend. A hook that fails after a suspension can run again when you resume.",
    "Review and repair a failing hook before resuming. Hooks must tolerate repeated calls and must not create new world objects.",
    "Keep the copy isolated for the whole operation. The connection check does not provide exclusive database access."
  };
  ...
  override description (owner: HACKER, flags: "rc") = "Help database for Snore Core utility objects and generics.";
  ...
  method find_topics owner: HACKER
    "Find stored topics and descriptions of core utility objects.";
    let what;
    let r;
    let w;
    if (!args)
    ...
    what = args[1];
    what[1] != "$" && return {};
    ts = pass("$generic_" + what[2..$]);
    ts && return ts;
    r = rindex(w = strsub(what[2..$], "-", "_"), "_utils");
    r && (r == length(w) - 5 && (`valid(#0.(w)) ! ANY' && `#0.(w):help_msg() ! ANY')) && return {what};
    return {};
  endmethod

  method get_topic owner: HACKER
    "Return utility descriptions for utility topics, otherwise use stored help.";
    const {topic, @rest} = args;
    if (length(topic) > 6 && topic == "$" + topic[2..$ - 5] + "utils")
      const name = strsub(topic[2..$], "-", "_");
      const object = `$sysobj.(name) ! E_PROPNF => $nothing';
      if (valid(object))
        const description = object:description();
        if (description)
          return {tostr("General information on $", name, ":"), "----", @typeof(description) == TYPE_STR ? {description} | description};
        endif
      endif
    endif
    return pass(@args);
  ...
  method dump_topic owner: #2
    "Return commands that recreate a stored topic or a core utility description.";
    const text = pass(@args);
    text != E_PROPNF && return text;
    const topic = args[1];
    topic[1] != "$" && return text;
    const name = topic[2..$];
    !(name in properties($sysobj)) && return text;
    const object = $sysobj.(name);
    typeof(object) != TYPE_OBJ && return text;
    const description = object.description;
    return {tostr(";;$", name, ".description = $command_utils:read_lines()"), @$command_utils:dump_lines(typeof(description) == TYPE_LIST ? description | {description})};
  endmethod
endobject
