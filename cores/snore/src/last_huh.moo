object LAST_HUH [
  ...
  method "@*" owner: #2
    ...
    const pos = "is" in args;
    if (pos == 1)
      player:notify(tostr("Usage:  ", verb, " <object> is <message>"));
      return;
    endif
    dobjstr = $string_utils:from_list(args[1..pos - 1], " ");
    const message = $string_utils:from_list(args[pos + 1..nargs], " ");
    const msg_name = verb[2..$];
    dobj = player:my_match_object(dobjstr);
    if ($command_utils:object_match_failed(dobj, dobjstr))
      "... oh well ...";
    elseif (pos == nargs)
      get = `dobj.(msg_name + "_msg") ! ANY';
      if (E_PROPNF == get)
      ...
      const set = dobj:set_message(msg_name, message);
      if (set)
